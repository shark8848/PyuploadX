...
- 完成结果；
- 下载链接。
- 文件浏览（缺省首页）：分页列出全部文件（§16.2 `GET /v1/files`），右侧支持状态
  过滤、名称/创建时间排序；行内仅图标操作（下载、复制下载链接、删除）；行首选择框
  支持跨页多选，顶部提供批量下载与批量删除，操作结果以右上角提示条反馈。
- 全局侧边栏：固定在窗体最左侧、贴边且可收缩隐藏（隐藏后菜单按钮恰好露出，不产生
  横向滚动）；“存储”标题旁提供新建桶（§16.8）与删除桶入口，删除桶需在弹窗中输入
  桶名确认，桶图标按桶内是否有 active 对象着色区分；底部操作区提供存储设置（§16.9，
  可配置默认桶、上传与生命周期参数，只读展示存储后端信息）、语言切换（中文/English）、
  明暗主题切换、退出登录；导航收缩后仅显示图标。
- 上传页：目标桶与前缀由侧边栏选择联动；上传队列按“进行中/已完成”分组，已完成历史
  默认折叠、只展示最近 20 条，可展开全部或一键清除（仅清理本地记录）；IndexedDB 中
  已完成记录最多保留 200 条，超出后自动清理更早记录。
- 界面国际化（i18n）与主题：Portal 支持中英文切换（默认中文，偏好保存在
  localStorage）与明暗主题切换（默认浅色）；切换按钮位于侧边栏底部操作区。
- 全局提示：所有操作反馈统一显示在右上角，带进度条、6 秒后自动消失（最多同时 3 条，
  悬停暂停计时）。
...
已完成记录（`fileUploads` 中 status=completed 的条目）在本地最多保留最近 200 条，
超出后自动清理更早记录；上传页“已完成历史”分组支持展开全部与一键清除，清除仅删除
本地记录，不影响服务端文件。
